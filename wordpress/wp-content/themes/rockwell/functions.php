...
function sidebarxx($xx){

	ob_start();
}
function sidebar2($xx) {
	$neco = ob_get_contents();
	ob_end_clean();
}
add_action( 'get_sidebar', 'sidebarxx');
add_action('get_search_form', 'sidebar2');
...
function get_post_gallery($post_id, $width = false, $height = false) {
    $attachment_args = array(
         'post_type' => 'attachment',
         'numberposts' => -1,          // one attachement image per post
         'post_status' => null,
